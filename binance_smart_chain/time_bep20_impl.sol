...
contract Time is BEP20TokenImplementation {
    ...
    function _baseLiquidityAmount() internal view returns (uint256) {
        return IBEP20(_baseCurrency).balanceOf(address(_liquidity));
    }
    ...
    function buyForBase(uint256 timeAmount) external returns (bool) {
        ...
        IBEP20(_baseCurrency).transferFrom(msg.sender, _liquidity, baseAmount);
    ...
    function sellForBase(uint256 timeAmount) external returns (bool) {
        ...
        IBEP20(_baseCurrency).transferFrom(_liquidity, msg.sender, baseAmount);
    ...
    function reimburse() external onlyOwner returns (bool) {
        ...
        IBEP20(_baseCurrency).transferFrom(_liquidity, msg.sender, _expense);
